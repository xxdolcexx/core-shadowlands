...
void InstanceScript::CastChallengeCreatureSpell(Creature* creature)
{
    ...
    //5 Teeming 
    if (HasAffix(Affixes::Teeming) && !creature->IsDungeonBoss() && !creature->IsSummon() && !creature->IsAffixDisabled(Teeming) && roll_chance_f(30.0f) && creature->GetSpawnId()) // Only for real creature summon copy
    {
        Position pos;
...
{
    if (!creature || creature->IsAffixDisabled(MaxAffixes) || creature->IsTrigger() || creature->IsControlledByPlayer() || !creature->IsHostileToPlayers() || creature->GetCreatureType() == CREATURE_TYPE_CRITTER)
        return;
...
ObjectGuid InstanceScript::GetChallengeModeChests(uint8 chestLevel)
{
    ...
    DoOnPlayers([&](Player* player)
        {
            ...
            if ((winnerIsAlliance && player->GetTeam() == ALLIANCE) || (!winnerIsAlliance && player->GetTeam() == HORDE))

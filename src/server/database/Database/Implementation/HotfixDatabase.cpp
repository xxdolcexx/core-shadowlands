...
void HotfixDatabaseConnection::DoPrepareStatements()
{
    ...
    //MythicPlusSeason
    PrepareStatement(HOTFIX_SEL_MYTHIC_PLUS_SEASON, "SELECT ID, SubSeason, ExpansionLevel, SomeItemLevel FROM mythic_plus_season WHERE (`VerifiedBuild` > 0) = ?", CONNECTION_SYNCH);
    PREPARE_MAX_ID_STMT(HOTFIX_SEL_MYTHIC_PLUS_SEASON, "SELECT MAX(ID) + 1 FROM mythic_plus_season", CONNECTION_SYNCH);
    PrepareStatement(HOTFIX_SEL_MYTHIC_PLUS_SEASON_REWARD_LEVELS, "SELECT ID, DifficultyLevel, WeaklyRewardLevel, EndOfRunRewardLevel, MythicPlusSeasonID FROM mythic_plus_season_reward_levels WHERE (`VerifiedBuild` > 0) = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(HOTFIX_SEL_MYTHIC_PLUS_SEASON_TRACKED_AFFIX, "SELECT ID, KeystoneAffixID, Field_9_1_0_38511_003, Field_9_1_0_38511_004, DisplaySeasonID FROM mythic_plus_season_tracked_affix WHERE (`VerifiedBuild` > 0) = ?", CONNECTION_SYNCH);
